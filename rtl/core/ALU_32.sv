...
always_comb begin
    ...
    // Synthesis-friendly OPCODE_EX decoding
    ...
    // Overflow Logic
    if (!prefix_active && OPCODE_EX == 4'h2) begin
        OF = (Aluin_A[31] == Aluin_B[31]) && (mux_out[31] != Aluin_A[31]);
    end else if (!prefix_active && OPCODE_EX == 4'h4) begin
        OF = (Aluin_A[31] == RB_Instant[31]) && (mux_out[31] != Aluin_A[31]);
    end else if (!prefix_active && OPCODE_EX == 4'h3) begin
        OF = (Aluin_A[31] != Aluin_B[31]) && (mux_out[31] != Aluin_A[31]);
    end else begin
        OF = 1'b0;
    end
